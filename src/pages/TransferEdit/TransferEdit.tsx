  ...
  const initialTransferEditValues = (values) => {
    ...
    if (values) {
      values.map((res) => {
        ...
        initailObj["transfer_title"] = res.transfer_title; // เรื่องที่บันทึก
        initailObj["transfer_member"] = res.transfer_member; // ผู้บันทึกข้อความ
        initailObj["transfer_date"] = res.transfer_date; // วันที่บันทึก
        initailObj["transfer_note"] = res.transfer_note; // รายละเอียด
        initailObj["transfer_file"] = res.transfer_file; // ไฟล์อัปโหลด
        return initailObj;
      });
    }
